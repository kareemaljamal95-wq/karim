...
1. Railway → service `ai-ceo` → **Settings → Networking → Custom Domain** → add
   the domain, copy the CNAME target
2. Registrar → DNS → replace the parking record with that CNAME
3. Railway → Variables → `MARKETING_DOMAIN=<your domain>` — without it the
   domain opens the dashboard instead of the public page
